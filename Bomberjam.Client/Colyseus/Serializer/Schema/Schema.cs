...
namespace Bomberjam.Client.Colyseus.Serializer.Schema
{
  internal class Context
  {
    private static readonly Lazy<Context> SingletonLazy = new Lazy<Context> (() => new Context());
    private readonly ConcurrentDictionary<uint, System.Type> typeIds = new ConcurrentDictionary<uint, System.Type>();
  ...
  public class ArraySchema<T> : ISchemaCollection
  {
    ...
    public void TriggerAll()
    {
      ...
      for (var i = 0; i < Items.Count; i++) {
        OnAdd.Invoke(this, new KeyValueEventArgs<T, int>(Items[i], i));
      }
    }
